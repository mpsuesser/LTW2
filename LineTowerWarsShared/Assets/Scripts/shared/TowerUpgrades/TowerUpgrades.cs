...
{
    private Dictionary<TowerType, HashSet<TowerUpgrade>> UpgradesAvailableToTower { get; set; }
    ...
    public HashSet<TowerUpgrade> GetAvailableUpgradesForTowerType(TowerType towerType) {
        if (UpgradesAvailableToTower.ContainsKey(towerType)) {
            return UpgradesAvailableToTower[towerType];
        }

        return new HashSet<TowerUpgrade>();
    }

    private void LoadTowerUpgrades() {
        UpgradesAvailableToTower = new Dictionary<TowerType, HashSet<TowerUpgrade>>();
        ...
        foreach (TowerUpgrade upgrade in upgrades) {
            if (!UpgradesAvailableToTower.ContainsKey(upgrade.SourceTowerType)) {
                UpgradesAvailableToTower[upgrade.SourceTowerType] = new HashSet<TowerUpgrade>();
            }
